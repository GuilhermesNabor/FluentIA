...
const PlacementTestPage = () => {
        ...
        getQuestions()
            .then(response => {
                if (Array.isArray(response.data) && response.data.length > 0) {
                    ...
                    setError('Não foi possível carregar as questões do teste. Tente atualizar a página.');
                }
            })
            .catch(err => {
                console.error("Erro ao buscar questões:", err);
                setError('Ocorreu um erro de comunicação com o servidor. Tente novamente mais tarde.');
            })
            .finally(() => {
                setIsLoading(false);
            });
    ...
    const handleSubmit = async () => {
        if (!Array.isArray(questions) || questions.length === 0) {
            setError('Não é possível finalizar um teste sem questões.');
            return;
        }
    ...
    if (error) return <div className="test-container"><p className="error">{error}</p></div>;
    if (testResult) {
        return (
    ...
    if (questions.length === 0) return <div className="test-container"><h2>Preparando seu teste...</h2></div>;
